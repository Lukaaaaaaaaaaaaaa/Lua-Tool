...
print(f"{y}[>{y}]{m} ENTER THE FILE PATH (It must be in .txt and it must contain 1 token per line): ")
global filePath, tokens, donetokenlist
filePath = input(f"{y}[--->{y}]{m} FILE PATH: ")
sort = "\""
for sort in sort:
...
def load_token():
    ...
    try:
        ...
        for token in tokenlist:
            ...
            loaded_amount += 1
        print(f"\n{y}[{Fore.LIGHTCYAN_EX}>{y}]{m} {loaded_amount} TOKENS LOADED")
        input(f"""\n{y}[>{y}]{m} PRITISNI ENTER ZA EXITE""")
    except:
        print(f"          {y}[{Fore.LIGHTCYAN_EX}>{y}]{m} FILE NIJE PRONADJE")
        input(f"""\n{y}[--->{y}]{m} PRITISNI ENTER ZA EXITE""")
        main()
...
def checkvalidity():
    ...
    for token in donetokenlist:
        while True:
            ...
            if "429" not in r1:
                break
            if "429" in r1:
                print(f'{y}[{Fore.LIGHTCYAN_EX}>{y}]{m} RATE LIMITED...')
                time.sleep(.3)
        r1 = str(r1)
    ...
    print(f"""\n
{y}[>{y}]{m} REZULTATI:\n
          {y}[{Fore.LIGHTCYAN_EX}>{y}]{m} DOBAR: {valid}""")
    if valid >=1:
        print("               You can find the list of valid tokens in output/valids.txt")
    print(f"""          {y}[{Fore.LIGHTCYAN_EX}>{y}]{m} VERIFICATION REQUIRED: {locked}
